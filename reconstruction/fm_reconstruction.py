...
class FMResynth:
    ...
    def synthesise(
        self, filename: str, curves: Dict[str, Oscillator], length: float = 1.0
    ):
        self._curves = curves
        lfo1 = ModSine(
            curves["v1"], amplitude=curves["B1"], samplerate=self._samplerate
        )
        s1 = ModSine(curves["f1"], amplitude=curves["A1"], fm_lfo=lfo1)

        lfo2 = ModSine(
            curves["v2"], amplitude=curves["B2"], samplerate=self._samplerate
        )
        s2 = ModSawtooth(curves["f2"], amplitude=curves["A2"], fm_lfo=lfo2)

        lfo3 = ModSine(
            curves["v3"], amplitude=curves["B3"], samplerate=self._samplerate
        )
        s3 = ModTriangle(curves["f3"], amplitude=curves["A3"], fm_lfo=lfo3)

        lfo4 = ModSine(
            curves["v4"], amplitude=curves["B4"], samplerate=self._samplerate
        )
    ...
    def create_model(
        self, model_file, parameters_file, input_file, window_size=16384, overlap=8
    ) -> Dict[str, Oscillator]:
        ...
        if not fs == 16384:
            print(
                f"Warning! Wrong Sample Rate: {fs} Models all work with 16834 at the moment"
            )
